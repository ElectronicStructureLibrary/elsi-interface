...
!> \param  a(lda,*)        Distributed matrix which should be factorized
...
!> \param  matrixCols      Number of local columns of matrix a
...
function elpa_cholesky_real_double(na,a,lda,nblk,matrixCols,mpi_comm_rows,&
  ...
  allocate(tmp1(nblk*nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_cholesky_real: error when allocating tmp1"
     stop
  endif

  allocate(tmp2(nblk,nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_cholesky_real: error when allocating tmp2"
     stop
  endif
  ...
  allocate(tmatr(l_rows,nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_cholesky_real: error when allocating tmatr"
     stop
  endif

  allocate(tmatc(l_cols,nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_cholesky_real: error when allocating tmatc"
     stop
  endif
  ...
  do n = 1,na,nblk
     ...
     do i=0,(na-1)/tile_size
        ...
        call DGEMM('N','T',lre-lrs+1,lce-lcs+1,nblk,-1.0_rk8,tmatr(lrs,1),ubound(tmatr,dim=1),&
                   tmatc(lcs,1),ubound(tmatc,dim=1),1.0_rk8,a(lrs,lcs),lda)
     enddo
  enddo
...
!> \param  na              Order of matrix
!> \param  a(lda,*)        Distributed matrix which should be inverted
!>                         Distribution is like in ScaLAPACK
!>                         Only upper triangle needs to be set
...
function elpa_invert_trm_real_double(na,a,lda,nblk,matrixCols,mpi_comm_rows,mpi_comm_cols,wantDebug) result(success)
  ...
  integer(kind=ik)    :: na,lda,nblk,matrixCols
  ...
  allocate(tmp1(nblk*nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_invert_trm_real: error when allocating tmp1"
     stop
  endif

  allocate(tmp2(nblk,nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_invert_trm_real: error when allocating tmp2"
     stop
  endif
  ...
  allocate(tmat1(l_rows,nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_invert_trm_real: error when allocating tmat1"
     stop
  endif

  allocate(tmat2(nblk,l_cols),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_invert_trm_real: error when allocating tmat2"
     stop
  endif
  ...
  do n = ns,1,-nblk
     ...
     if(l_row1>1 .and. l_cols-l_col1+1>0) &
        call dgemm('N','N',l_row1-1,l_cols-l_col1+1,nb,-1.0_rk8,tmat1,ubound(tmat1,dim=1),tmat2(1,l_col1),&
                   ubound(tmat2,dim=1),1.0_rk8,a(1,l_col1),lda)
  enddo
...
function elpa_cholesky_complex_double(na,a,lda,nblk,matrixCols,mpi_comm_rows,mpi_comm_cols,wantDebug) result(success)
  ...
  allocate(tmp1(nblk*nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_cholesky_complex: error when allocating tmp1"
     stop
  endif

  allocate(tmp2(nblk,nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_cholesky_complex: error when allocating tmp2"
     stop
  endif
  ...
  allocate(tmatr(l_rows,nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_cholesky_complex: error when allocating tmatr"
     stop
  endif

  allocate(tmatc(l_cols,nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_cholesky_complex: error when allocating tmatc"
     stop
  endif
  ...
  do n = 1,na,nblk
     ...
     ! this has to be checked since it was changed substantially when doing type safe
     call elpa_transpose_vectors_complex_double(tmatc,ubound(tmatc,dim=1),mpi_comm_cols,tmatr,&
                                                ubound(tmatr,dim=1),mpi_comm_rows,n,na,nblk,nblk)
     ...
     do i=0,(na-1)/tile_size
        ...
        call ZGEMM('N','C',lre-lrs+1,lce-lcs+1,nblk,(-1.0_rk8,0.0_rk8),tmatr(lrs,1),&
                   ubound(tmatr,dim=1),tmatc(lcs,1),ubound(tmatc,dim=1),(1.0_rk8,0.0_rk8),&
                   a(lrs,lcs),lda)
     enddo
...
function elpa_invert_trm_complex_double(na,a,lda,nblk,matrixCols,mpi_comm_rows,mpi_comm_cols,wantDebug) result(success)
  ...
  allocate(tmp1(nblk*nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_invert_trm_complex: error when allocating tmp1"
     stop
  endif

  allocate(tmp2(nblk,nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_invert_trm_complex: error when allocating tmp2"
     stop
  endif
  ...
  allocate(tmat1(l_rows,nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_invert_trm_complex: error when allocating tmat1"
     stop
  endif

  allocate(tmat2(nblk,l_cols),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_invert_trm_complex: error when allocating tmat2"
     stop
  endif
  ...
  do n = ns,1,-nblk
     ...
     if(my_prow==prow(n,nblk,np_rows)) then
        ...
        if(l_cols-l_colx+1>0)&
           call ZTRMM('L','U','N','N',nb,l_cols-l_colx+1,(1.0_rk8,0.0_rk8),tmp2,ubound(tmp2,dim=1),&
                      a(l_row1,l_colx),lda)
     ...
     if(l_row1>1 .and. l_cols-l_col1+1>0)&
        call ZGEMM('N','N',l_row1-1,l_cols-l_col1+1,nb,(-1.0_rk8,0.0_rk8),tmat1,ubound(tmat1,dim=1),&
                   tmat2(1,l_col1),ubound(tmat2,dim=1),(1.0_rk8,0.0_rk8),a(1,l_col1),lda)
  enddo
...
!>                         Even when uplo_c is 'U' or 'L', the other triangle may be written
!>                         to a certain extent, i.e. one shouldn't rely on the content there
!>
...
function elpa_mult_at_b_real_double(uplo_a,uplo_c,na,ncb,a,lda,ldaCols,b,ldb,ldbCols,nblk,&
  ...
  integer(kind=ik), intent(in) :: na,ncb,lda,ldb,nblk,ldc
  integer(kind=ik), intent(in) :: ldaCols,ldbCols,ldcCols
  integer(kind=ik)             :: mpi_comm_rows,mpi_comm_cols
  ...
  allocate(aux_mat(l_rows,nblk_mult),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_mult_at_b_real: error when allocating aux_mat"
     stop
  endif

  allocate(aux_bc(l_rows*nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_mult_at_b_real: error when allocating aux_bc"
     stop
  endif

  allocate(lrs_save(nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_mult_at_b_real: error when allocating lrs_save"
     stop
  endif

  allocate(lre_save(nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_mult_at_b_real: error when allocating lre_save"
     stop
  endif
  ...
  do np = 0,np_rows-1
     ...
     do nb=0,(l_rows_np-1)/nblk
        ...
        if(nstor==nblk_mult .or. nb*nblk+nblk >= l_rows_np) then
           ...
           if(lcs<=lce) then
              allocate(tmp1(nstor,lcs:lce),tmp2(nstor,lcs:lce),stat=istat)
              if(istat .ne. 0) then
                 print *,"elpa_mult_at_b_real: error when allocating tmp1"
                 stop
              endif

              if(lrs<=lre) then
                 call dgemm('T','N',nstor,lce-lcs+1,lre-lrs+1,1.0_rk8,aux_mat(lrs,1),&
                            ubound(aux_mat,dim=1),b(lrs,lcs),ldb,0.0_rk8,tmp1,nstor)
              else
                 tmp1 = 0
              endif

              ! Sum up the results and send to processor row np
              call mpi_reduce(tmp1,tmp2,nstor*(lce-lcs+1),MPI_REAL8,MPI_SUM,np,&
                              mpi_comm_rows,mpierr)
...
function elpa_mult_ah_b_complex_double(uplo_a,uplo_c,na,ncb,a,lda,ldaCols,b,ldb,ldbCols,nblk,&
  ...
  allocate(aux_mat(l_rows,nblk_mult),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_mult_ah_b_complex: error when allocating aux_mat"
     stop
  endif

  allocate(aux_bc(l_rows*nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_mult_ah_b_complex: error when allocating aux_bc"
     stop
  endif

  allocate(lrs_save(nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_mult_ah_b_complex: error when allocating lrs_save"
     stop
  endif

  allocate(lre_save(nblk),stat=istat)
  if(istat .ne. 0) then
     print *,"elpa_mult_ah_b_complex: error when allocating lre_save"
     stop
  endif
  ...
  do np = 0,np_rows-1
     ...
     do nb=0,(l_rows_np-1)/nblk
        ...
        if(nstor==nblk_mult .or. nb*nblk+nblk >= l_rows_np) then
           ...
           if(lcs<=lce) then
              allocate(tmp1(nstor,lcs:lce),stat=istat)
              allocate(tmp2(nstor,lcs:lce),stat=istat)
              if(istat .ne. 0) then
                 print *,"elpa_mult_ah_b_complex: error when allocating tmp1,tmp2"
                 stop
              endif

              if(lrs<=lre) then
                 call zgemm('C','N',nstor,lce-lcs+1,lre-lrs+1,(1.0_rk8,0.0_rk8),aux_mat(lrs,1),&
                            ubound(aux_mat,dim=1),b(lrs,lcs),ldb,(0.0_rk8,0.0_rk8),tmp1,nstor)
              else
                 tmp1 = 0
              endif
...
!> \param  matrixCols      Number of columns of matrix q
!> \param  mpi_comm_rows   MPI communicator for rows
!> \param  mpi_comm_cols   MPI communicator for columns
!> \param  wantDebug       Logical, give more debug information if .true.
!> \result success         Logical, .true. on success, else .false.

function elpa_solve_tridi_double(na,nev,d,e,q,ldq,nblk,matrixCols,mpi_comm_rows,&
                                 mpi_comm_cols,wantDebug) result(success)
  ...
  call solve_tridi_double_private(na,nev,d,e,q,ldq,nblk,matrixCols,mpi_comm_rows,&
