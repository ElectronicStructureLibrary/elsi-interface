...
static __attribute__((always_inline)) void hh_trafo_complex_kernel_24_AVX2_1hv_single(float _Complex* q, float _Complex* hh, int nb, int ldq 
		                       );




static __attribute__((always_inline)) void hh_trafo_complex_kernel_20_AVX2_1hv_single(float _Complex* q, float _Complex* hh, int nb, int ldq
		                       );
...
static __attribute__((always_inline)) void hh_trafo_complex_kernel_16_AVX2_1hv_single(float _Complex* q, float _Complex* hh, int nb, int ldq
		                       );




static __attribute__((always_inline)) void hh_trafo_complex_kernel_12_AVX2_1hv_single(float _Complex* q, float _Complex* hh, int nb, int ldq
		                       );




static __attribute__((always_inline)) void hh_trafo_complex_kernel_8_AVX2_1hv_single(float _Complex* q, float _Complex* hh, int nb, int ldq
		                       );




static __attribute__((always_inline)) void hh_trafo_complex_kernel_4_AVX2_1hv_single(float _Complex* q, float _Complex* hh, int nb, int ldq
		                       );
...
void single_hh_trafo_complex_AVX2_1hv_single (float _Complex* q, float _Complex* hh, int* pnb, int* pnq, int* pldq
		  )
{
     ...
     worked_on = 0;
        ...
        {
            hh_trafo_complex_kernel_24_AVX2_1hv_single (&q[i], hh, nb, ldq);
	    worked_on += 24;
        }
        ...
        if (nq-i == 20)
        {
            hh_trafo_complex_kernel_20_AVX2_1hv_single (&q[i], hh, nb, ldq);
	    worked_on += 20;
        }
        ...
        if (nq-i == 16)
        {
            hh_trafo_complex_kernel_16_AVX2_1hv_single (&q[i], hh, nb, ldq);
	    worked_on += 16;
        }
        ...
        if (nq-i == 12)
        {
            hh_trafo_complex_kernel_12_AVX2_1hv_single (&q[i], hh, nb, ldq);
	    worked_on += 12;
        }
        ...
        if (nq-i == 8)
        {
            hh_trafo_complex_kernel_8_AVX2_1hv_single (&q[i], hh, nb, ldq);
	    worked_on += 8;
        }
        ...
        if (nq-i == 4)
        {
            hh_trafo_complex_kernel_4_AVX2_1hv_single (&q[i], hh, nb, ldq);
	    worked_on += 4;
        }
		...
		)
{
    ...
    x1 = _mm256_load_ps(&q_dbl[0]);
    x2 = _mm256_load_ps(&q_dbl[8]);
    x3 = _mm256_load_ps(&q_dbl[2*8]);
    x4 = _mm256_load_ps(&q_dbl[3*8]);
    x5 = _mm256_load_ps(&q_dbl[4*8]);
    x6 = _mm256_load_ps(&q_dbl[5*8]);
    ...
    for (i = 1; i < nb; i++)
    {
       ...
       h1_real = _mm256_broadcast_ss(&hh_dbl[(i-1+1)*2]);
       h1_imag = _mm256_broadcast_ss(&hh_dbl[((i-1+1)*2)+1]);



        q1 = _mm256_load_ps(&q_dbl[(2*i*ldq)+0]);
        q2 = _mm256_load_ps(&q_dbl[(2*i*ldq)+8]);
        q3 = _mm256_load_ps(&q_dbl[(2*i*ldq)+2*8]);
        q4 = _mm256_load_ps(&q_dbl[(2*i*ldq)+3*8]);
        q5 = _mm256_load_ps(&q_dbl[(2*i*ldq)+4*8]);
        q6 = _mm256_load_ps(&q_dbl[(2*i*ldq)+5*8]);
        ...
        tmp4 = _mm256_mul_ps(  h1_imag, q4);
        x4 = _mm256_add_ps(  x4, _mm256_fmsubadd_ps(h1_real,q4,_mm256_shuffle_ps(tmp4, tmp4, 0xb1)));

        tmp5 = _mm256_mul_ps(  h1_imag, q5);
        x5 = _mm256_add_ps(  x5, _mm256_fmsubadd_ps(h1_real,q5,_mm256_shuffle_ps(tmp5, tmp5, 0xb1)));

        tmp6 = _mm256_mul_ps(  h1_imag, q6);
        x6 = _mm256_add_ps(  x6, _mm256_fmsubadd_ps(h1_real,q6,_mm256_shuffle_ps(tmp6, tmp6, 0xb1)));
    ...
    tmp4 = _mm256_mul_ps(  h1_imag, x4);
    x4 = _mm256_fmaddsub_ps(h1_real,x4,_mm256_shuffle_ps(tmp4, tmp4, 0xb1));

    tmp5 = _mm256_mul_ps(  h1_imag, x5);
    x5 = _mm256_fmaddsub_ps(h1_real,x5,_mm256_shuffle_ps(tmp5, tmp5, 0xb1));

    tmp6 = _mm256_mul_ps(  h1_imag, x6);
    x6 = _mm256_fmaddsub_ps(h1_real,x6,_mm256_shuffle_ps(tmp6, tmp6, 0xb1));


    q1 = _mm256_load_ps(&q_dbl[0]);
    q2 = _mm256_load_ps(&q_dbl[8]);
    q3 = _mm256_load_ps(&q_dbl[2*8]);
    q4 = _mm256_load_ps(&q_dbl[3*8]);
    q5 = _mm256_load_ps(&q_dbl[4*8]);
    q6 = _mm256_load_ps(&q_dbl[5*8]);

    q1 = _mm256_add_ps(  q1, x1);
    q2 = _mm256_add_ps(  q2, x2);
    q3 = _mm256_add_ps(  q3, x3);
    q4 = _mm256_add_ps(  q4, x4);
    q5 = _mm256_add_ps(  q5, x5);
    q6 = _mm256_add_ps(  q6, x6);


    _mm256_store_ps(&q_dbl[0], q1);
    _mm256_store_ps(&q_dbl[8], q2);
    _mm256_store_ps(&q_dbl[2*8], q3);
    _mm256_store_ps(&q_dbl[3*8], q4);
    _mm256_store_ps(&q_dbl[4*8], q5);
    _mm256_store_ps(&q_dbl[5*8], q6);
	...
	h1_real = _mm256_broadcast_ss(&hh_dbl[(i-1+1)*2]);
        ...
        q3 = _mm256_add_ps(  q3, _mm256_fmaddsub_ps(h1_real,x3,_mm256_shuffle_ps(tmp3, tmp3, 0xb1)));
         ...
         tmp4 = _mm256_mul_ps(  h1_imag, x4);
         q4 = _mm256_add_ps(  q4, _mm256_fmaddsub_ps(h1_real,x4,_mm256_shuffle_ps(tmp4, tmp4, 0xb1)));

         tmp5 = _mm256_mul_ps(  h1_imag, x5);
         q5 = _mm256_add_ps(  q5, _mm256_fmaddsub_ps(h1_real,x5,_mm256_shuffle_ps(tmp5, tmp5, 0xb1)));

         tmp6 = _mm256_mul_ps(  h1_imag, x6);
         q6 = _mm256_add_ps(  q6, _mm256_fmaddsub_ps(h1_real,x6,_mm256_shuffle_ps(tmp6, tmp6, 0xb1)));



         _mm256_store_ps(&q_dbl[(2*i*ldq)+0], q1);
         _mm256_store_ps(&q_dbl[(2*i*ldq)+8], q2);
         _mm256_store_ps(&q_dbl[(2*i*ldq)+2*8], q3);
         _mm256_store_ps(&q_dbl[(2*i*ldq)+3*8], q4);
         _mm256_store_ps(&q_dbl[(2*i*ldq)+4*8], q5);
         _mm256_store_ps(&q_dbl[(2*i*ldq)+5*8], q6);
    }
		...
		)
{
    ...
    __m256 x1, x2, x3, x4, x5;
    __m256 q1, q2, q3, q4, q5;
    __m256 h1_real, h1_imag;
    __m256 tmp1, tmp2, tmp3, tmp4, tmp5;
    int i=0;
