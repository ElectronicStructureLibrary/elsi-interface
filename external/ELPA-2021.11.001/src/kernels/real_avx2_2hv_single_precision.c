...
__attribute__((always_inline)) static void hh_trafo_kernel_8_AVX2_2hv_single (float* q, float* hh, int nb, int ldq, int ldh, 
	float s);



__attribute__((always_inline)) static void hh_trafo_kernel_16_AVX2_2hv_single (float* q, float* hh, int nb, int ldq, int ldh, 
	float s);




__attribute__((always_inline)) static void hh_trafo_kernel_24_AVX2_2hv_single (float* q, float* hh, int nb, int ldq, int ldh,
	float s);




__attribute__((always_inline)) static void hh_trafo_kernel_32_AVX2_2hv_single (float* q, float* hh, int nb, int ldq, int ldh, 
	float s);




__attribute__((always_inline)) static void hh_trafo_kernel_40_AVX2_2hv_single (float* q, float* hh, int nb, int ldq, int ldh, 
	float s);




__attribute__((always_inline)) static void hh_trafo_kernel_48_AVX2_2hv_single (float* q, float* hh, int nb, int ldq, int ldh,
	float s);

void double_hh_trafo_real_AVX2_2hv_single (float* q, float* hh, int* pnb, int* pnq, int* pldq, int* pldh);
...
void double_hh_trafo_real_AVX2_2hv_single (float* q, float* hh, int* pnb, int* pnq, int* pldq, int* pldh)
{
  ...
  for (i = 0; i < nq - 40; i+= 48 )
    {
      hh_trafo_kernel_48_AVX2_2hv_single (&q[i], hh, nb, ldq, ldh, s);
      worked_on += 48;
    }
  ...
  if (nq-i == 40)
    {
      hh_trafo_kernel_40_AVX2_2hv_single (&q[i], hh, nb, ldq, ldh, s);
      worked_on += 40;
    }
  ...
  if (nq-i == 32)
    {
      hh_trafo_kernel_32_AVX2_2hv_single (&q[i], hh, nb, ldq, ldh, s);
      worked_on += 32;
    }
  ...
  if (nq-i == 24)
    {
      hh_trafo_kernel_24_AVX2_2hv_single (&q[i], hh, nb, ldq, ldh, s);
      worked_on += 24;
    }
  ...
  if (nq-i == 16)
    {
      hh_trafo_kernel_16_AVX2_2hv_single (&q[i], hh, nb, ldq, ldh, s);
      worked_on += 16;
    }
  ...
  if (nq-i == 8)
    {
      hh_trafo_kernel_8_AVX2_2hv_single (&q[i], hh, nb, ldq, ldh, s);
      worked_on += 8;
    }
...
__attribute__((always_inline)) static void hh_trafo_kernel_48_AVX2_2hv_single (float* q, float* hh, int nb, int ldq, int ldh,
               float s)
  {
    ...
    __m256 x3 = _mm256_load_ps(&q[ldq+2*8]);
    __m256 x4 = _mm256_load_ps(&q[ldq+3*8]);
    __m256 x5 = _mm256_load_ps(&q[ldq+4*8]);
    __m256 x6 = _mm256_load_ps(&q[ldq+5*8]);

    __m256 h1 = _mm256_broadcast_ss(&hh[ldh+1]);
 
    __m256 h2;
    __m256 q1 = _mm256_load_ps(q);
    __m256 y1 = _mm256_fmadd_ps(x1,h1,q1);
    __m256 q2 = _mm256_load_ps(&q[8]);
    __m256 y2 = _mm256_fmadd_ps(x2,h1,q2);
    __m256 q3 = _mm256_load_ps(&q[2*8]);
    __m256 y3 = _mm256_fmadd_ps(x3,h1,q3);
    __m256 q4 = _mm256_load_ps(&q[3*8]);
    __m256 y4 = _mm256_fmadd_ps(x4,h1,q4);
    __m256 q5 = _mm256_load_ps(&q[4*8]);
    __m256 y5 = _mm256_fmadd_ps(x5,h1,q5);
    __m256 q6 = _mm256_load_ps(&q[5*8]);
    __m256 y6 = _mm256_fmadd_ps(x6,h1,q6);
    ...
    for(i = 2; i < nb; i++)
      {
        ...
        h1 = _mm256_broadcast_ss(&hh[i-(2-1)]);
        h2 = _mm256_broadcast_ss(&hh[ldh+i-(2-2)]);

        q1 = _mm256_load_ps(&q[i*ldq]);
        q2 = _mm256_load_ps(&q[(i*ldq)+8]);
        q3 = _mm256_load_ps(&q[(i*ldq)+2*8]);
        q4 = _mm256_load_ps(&q[(i*ldq)+3*8]);
        q5 = _mm256_load_ps(&q[(i*ldq)+4*8]);
        q6 = _mm256_load_ps(&q[(i*ldq)+5*8]);

        x1 = _mm256_fmadd_ps(q1,h1,x1);
        y1 = _mm256_fmadd_ps(q1,h2,y1);
        x2 = _mm256_fmadd_ps(q2,h1,x2);
        y2 = _mm256_fmadd_ps(q2,h2,y2);
        x3 = _mm256_fmadd_ps(q3,h1,x3);
        y3 = _mm256_fmadd_ps(q3,h2,y3);
        x4 = _mm256_fmadd_ps(q4,h1,x4);
        y4 = _mm256_fmadd_ps(q4,h2,y4);
        x5 = _mm256_fmadd_ps(q5,h1,x5);
        y5 = _mm256_fmadd_ps(q5,h2,y5);
        x6 = _mm256_fmadd_ps(q6,h1,x6);
        y6 = _mm256_fmadd_ps(q6,h2,y6);


      }
    h1 = _mm256_broadcast_ss(&hh[nb-(2-1)]);

    q1 = _mm256_load_ps(&q[nb*ldq]);
    q2 = _mm256_load_ps(&q[(nb*ldq)+8]);
    q3 = _mm256_load_ps(&q[(nb*ldq)+2*8]);
    q4 = _mm256_load_ps(&q[(nb*ldq)+3*8]);
    q5 = _mm256_load_ps(&q[(nb*ldq)+4*8]);
    q6 = _mm256_load_ps(&q[(nb*ldq)+5*8]);
    x1 = _mm256_fmadd_ps(q1,h1,x1);
    x2 = _mm256_fmadd_ps(q2,h1,x2);
    x3 = _mm256_fmadd_ps(q3,h1,x3);
    x4 = _mm256_fmadd_ps(q4,h1,x4);
   ...
   y1 = _mm256_fmadd_ps(y1,h1,_mm256_mul_ps( x1,h2));
   y2 = _mm256_fmadd_ps(y2,h1,_mm256_mul_ps( x2,h2));
   y3 = _mm256_fmadd_ps(y3,h1,_mm256_mul_ps( x3,h2));
   y4 = _mm256_fmadd_ps(y4,h1,_mm256_mul_ps( x4,h2));
   y5 = _mm256_fmadd_ps(y5,h1,_mm256_mul_ps( x5,h2));
   y6 = _mm256_fmadd_ps(y6,h1,_mm256_mul_ps( x6,h2));
   ...
   q3 = _mm256_load_ps(&q[2*8]);
   q3 = _mm256_add_ps(  q3, y3);
   _mm256_store_ps(&q[2*8], q3);
   q4 = _mm256_load_ps(&q[3*8]);
   q4 = _mm256_add_ps(  q4, y4);
   _mm256_store_ps(&q[3*8], q4);
   q5 = _mm256_load_ps(&q[4*8]);
   q5 = _mm256_add_ps(  q5, y5);
   _mm256_store_ps(&q[4*8], q5);
   q6 = _mm256_load_ps(&q[5*8]);
   q6 = _mm256_add_ps(  q6, y6);
   _mm256_store_ps(&q[5*8], q6);

   h2 = _mm256_broadcast_ss(&hh[ldh+1]);

   q1 = _mm256_load_ps(&q[ldq]);
   q2 = _mm256_load_ps(&q[ldq+8]);
   q3 = _mm256_load_ps(&q[ldq+2*8]);
   q4 = _mm256_load_ps(&q[ldq+3*8]);
   q5 = _mm256_load_ps(&q[ldq+4*8]);
   q6 = _mm256_load_ps(&q[ldq+5*8]);
   q1 = _mm256_add_ps(  q1, _mm256_fmadd_ps(y1,h2,x1));
   q2 = _mm256_add_ps(  q2, _mm256_fmadd_ps(y2,h2,x2));
   q3 = _mm256_add_ps(  q3, _mm256_fmadd_ps(y3,h2,x3));
   q4 = _mm256_add_ps(  q4, _mm256_fmadd_ps(y4,h2,x4));
   q5 = _mm256_add_ps(  q5, _mm256_fmadd_ps(y5,h2,x5));
   q6 = _mm256_add_ps(  q6, _mm256_fmadd_ps(y6,h2,x6));
   _mm256_store_ps(&q[ldq], q1);
   _mm256_store_ps(&q[ldq+8], q2);
   _mm256_store_ps(&q[ldq+2*8], q3);
   _mm256_store_ps(&q[ldq+3*8], q4);
   _mm256_store_ps(&q[ldq+4*8], q5);
   _mm256_store_ps(&q[ldq+5*8], q6);
   ...
   for (i = 2; i < nb; i++)
   {
    ...
    h2 = _mm256_broadcast_ss(&hh[ldh+i-(2-2)]);
     ...
     q1 = _mm256_fmadd_ps(x1,h1,q1);
     q1 = _mm256_fmadd_ps(y1,h2,q1);
     q2 = _mm256_fmadd_ps(x2,h1,q2);
     q2 = _mm256_fmadd_ps(y2,h2,q2);
     q3 = _mm256_fmadd_ps(x3,h1,q3);
     q3 = _mm256_fmadd_ps(y3,h2,q3);
     q4 = _mm256_fmadd_ps(x4,h1,q4);
     q4 = _mm256_fmadd_ps(y4,h2,q4);
     q5 = _mm256_fmadd_ps(x5,h1,q5);
     q5 = _mm256_fmadd_ps(y5,h2,q5);
     q6 = _mm256_fmadd_ps(x6,h1,q6);
     q6 = _mm256_fmadd_ps(y6,h2,q6);



     _mm256_store_ps(&q[i*ldq], q1);
     _mm256_store_ps(&q[(i*ldq)+8], q2);
     _mm256_store_ps(&q[(i*ldq)+2*8], q3);
     _mm256_store_ps(&q[(i*ldq)+3*8], q4);
     _mm256_store_ps(&q[(i*ldq)+4*8], q5);
     _mm256_store_ps(&q[(i*ldq)+5*8], q6);
   ...
   _mm256_store_ps(&q[(nb*ldq)+2*8], q3);
   _mm256_store_ps(&q[(nb*ldq)+3*8], q4);
   _mm256_store_ps(&q[(nb*ldq)+4*8], q5);
   _mm256_store_ps(&q[(nb*ldq)+5*8], q6);
