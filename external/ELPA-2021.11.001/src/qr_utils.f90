...
subroutine reverse_matrix_local_&
    ...
    ! local scalars
    real(kind=c_double)    :: dworksize(1)
...
subroutine reverse_matrix_2dcomm_ref_&
    ...
    ! local scalars
    real(kind=c_double)    :: reverse_column_size(1)
    real(kind=c_double)    :: reverse_row_size(1)
    ...
    do while (current_index .le. dimsize)
        ...
        (0,lsize,lcols,work(sendoffset),lsize,work(work_offset),lwork)
            ...
            if (dest_process .eq. mpirank) then
                ...
                call MPI_Recv(work(recvoffset), int(recvcount,kind=MPI_KIND), MPI_REAL8, &
        ...
        (0,lsize,lcols,work(recvoffset),lsize,work(work_offset),lwork)
            ...
            if (src_process .eq. mpirank) then
                ...
                ! 6a. call MPI_Send
                call MPI_Send(work(sendoffset), int(sendcount,kind=MPI_KIND), MPI_REAL8, &
...
subroutine reverse_matrix_2dcomm_ref_&
    ...
    ! local scalars
    real(kind=c_float)    :: reverse_column_size(1)
    real(kind=c_float)    :: reverse_row_size(1)
    ...
    do while (current_index .le. dimsize)
        ...
        (0,lsize,lcols,work(sendoffset),lsize,work(work_offset),lwork)
            ...
            if (dest_process .eq. mpirank) then
                ...
                call MPI_Recv(work(recvoffset), int(recvcount,kind=MPI_KIND), MPI_REAL4, &
                              int(src_process,kind=MPI_KIND), int(current_index,kind=MPI_KIND),   &
                              int(mpicomm,kind=MPI_KIND), my_mpistatus, mpierr)
        ...
        (0,lsize,lcols,work(recvoffset),lsize,work(work_offset),lwork)
            ...
            if (src_process .eq. mpirank) then
                ...
                ! 6a. call MPI_Send
                call MPI_Send(work(sendoffset), int(sendcount,kind=MPI_KIND), MPI_REAL4, &
                              int(dest_process,kind=MPI_KIND), int(current_index,kind=MPI_KIND),  &
                              int(mpicomm,kind=MPI_KIND), mpierr)
            end if
        end if
