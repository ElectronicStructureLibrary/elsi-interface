   ...
   if (gpu .eq. 1) then
    ...
    if (useGPU) then
      ...
      if (nblk*(max(np_rows,np_cols)-1) >= na) then
        ...
        do_useGPU = .false.
        if (kernel == ELPA_2STAGE_REAL_NVIDIA_GPU) then
  ...
  ! have to point ev
   ...
   if (nev == 0) then
    ...
    if (.not. obj%eigenvalues_only) then
     ...
     call check_allocate_f("elpa2_template: q_dummy", 1032,  istat,  errorMessage)
    ...
    if (obj%is_set("bandwidth") == 1) then
      ...
      call check_allocate_f("elpa2_template: tmat", 1120,  istat,  errorMessage)
    ...
    if (do_bandred) then
     ...
     if (do_tridiag) then
       ...
       call check_allocate_f("elpa2_template: e", 1160,  istat,  errorMessage)
...
!        print *, 'do_useGPU_solve_tridi=', do_useGPU_solve_tridi
     ...
     call check_deallocate_f("elpa2_template: e", 1236,  istat,  errorMessage)
...
! #endif
     ...
     if (do_trans_to_band) then
       ...
       call check_deallocate_f("elpa2_template: hh_trans", 1385,  istat,  errorMessage)
     endif
     ...
     if (allocated(tmat)) then
       ...
       call check_deallocate_f("elpa2_template: tmat", 1417,  istat,  errorMessage)
     endif

     if (obj%eigenvalues_only) then
       deallocate(q_dummy, stat=istat, errmsg=errorMessage)
       call check_deallocate_f("elpa2_template: q_dummy", 1422,  istat,  errorMessage)
     endif
...
! intel missing
   ...
   call check_memcpy_GPU_f("elpa1: aExtern -> aIntern", 612,  successGPU)
...
! #endif
   ...
   call check_memcpy_GPU_f("elpa1: qIntern -> qExtern", 1476,  successGPU)
   successGPU = gpu_memcpy(evExtern, c_loc(ev(1)), obj%na*size_of_datatype, &
                             gpuMemcpyHostToDevice)
   call check_memcpy_GPU_f("elpa1: ev -> evExtern", 1479,  successGPU)
   ...
   if (gpu .eq. 1) then
    ...
    if (useGPU) then
      ...
      if (nblk*(max(np_rows,np_cols)-1) >= na) then
        ...
        do_useGPU = .false.
        if (kernel == ELPA_2STAGE_COMPLEX_NVIDIA_GPU) then
          if (userHasSetKernel) then
            ! user fixed inconsistent input.
            ! sadly, we do have to abort
            if (my_pe .eq. 0) then
  ...
  ! have to point ev
   ...
   if (nev == 0) then
    ...
    if (do_bandred) then
     ...
     call check_allocate_f("elpa2_template: q_real", 1203,  istat,  errorMessage)
...
!        print *, 'do_useGPU_solve_tridi=', do_useGPU_solve_tridi
     ...
     if (allocated(q_real)) then
       ...
       call check_deallocate_f("elpa2_template: q_real", 1284,  istat,  errorMessage)
     endif
