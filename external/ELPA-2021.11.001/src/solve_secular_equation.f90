...
!
  ...
  if (i==n) then
   ...
   a = 0.0_c_double ! delta(n)
   b = rho*SUM(z(:)**2) + 1.0_c_double ! rho*SUM(z(:)**2) is the lower bound for the guess
  else
    ...
    x = 0.5_c_double*(d(i)+d(i+1))
    y = 1.0_c_double + rho*SUM(z(:)**2/(d(:)-x))
    if (y>0) then
...
!
  ...
  if (i==n) then
   ...
   a = 0.0_c_float ! delta(n)
   b = rho*SUM(z(:)**2) + 1.0_c_float ! rho*SUM(z(:)**2) is the lower bound for the guess
  else

    ! Other eigenvalues: lower bound is d(i), upper bound is d(i+1)
    ! We check the sign of the function in the midpoint of the interval
    ! in order to determine if eigenvalue is more close to d(i) or d(i+1)
    x = 0.5_c_float*(d(i)+d(i+1))
    y = 1.0_c_float + rho*SUM(z(:)**2/(d(:)-x))
    if (y>0) then
      ! solution is next to d(i)
      dshift = d(i)
    else
