...
!  Transforms the eigenvectors of a tridiagonal matrix back to the
!  eigenvectors of the band matrix
!
!  Parameters
!
...
!
!  nblk        blocksize of cyclic distribution, must be the same in
!  both directions!
!
!  nb          semi bandwith
!
...
!    use cudafor
    ...
    integer:: istat
    complex*16, allocatable :: row(:),row_group(:,:)
    ...
    real*8 ttt0, ttt1, ttt2, t2_compute_kernel, t0_compute_kernel,t1_compute_kernel, t0_mpi_time, t1_mpi_time,t2_mpi_time
    real*8 t0_cpu_code,t1_cpu_code,t2_cpu_code,t0_block_time,t1_block_time,t2_block_time,t0_cuda_memcpy

    real*8 t0_inner_do_time, t1_inner_do_time , t2_inner_do_time,t0_outer_do_time ,t1_outer_do_time ,t2_outer_do_time ,t0_result_time ,t1_result_time, t2_result_time,t0_mpi_recv_time, t1_mpi_recv_time,t2_mpi_recv_time

   real*8 t1_mpi_wait_time,t0_mpi_wait_time,t2_mpi_wait_time,t1_memcpy_time,t0_memcpy_time,t2_memcpy_time,t1_mpi_irecv_time,t0_mpi_irecv_time,t2_mpi_irecv_time,t0_mpi_outer_wait_time,t1_mpi_outer_wait_time,t2_mpi_outer_wait_time, time0
!    real*4 time1
...
!    type(cudaEvent) :: start, stop
    ...
    if(l_nev==0) then
        ...
        ! Suggested stripe width is 48 - should this be reduced for the
        ! complex case ???
!!        stripe_width = 48 ! Must be a multiple of 4
        stripe_width = 256 
        stripe_count = (l_nev-1)/stripe_width + 1
        ! Adapt stripe width so that last one doesn't get too small
!NOTE(ca): I commented the following two lines
...
!        stripe_width = ((stripe_width+3)/4)*4 ! Must be a multiple of 4
    ...
    ! Copy q from a block cyclic distribution into a distribution with
    ! contiguous rows,
    ! and transpose the matrix using stripes of given stripe_width for
    ! cache blocking.

    ! The peculiar way it is done below is due to the fact that the last
    ! row should be
...
!        t2_mpi_irecv_time =MPI_Wtime()
!        t0_mpi_irecv_time = t0_mpi_irecv_time + ( t2_mpi_irecv_time - t1_mpi_irecv_time)
...
!               time0 = time0 + time1
               ...
               t0_mpi_wait_time = t0_mpi_wait_time + ( t2_mpi_wait_time-t1_mpi_wait_time)
                ...
                dev_offset = (0 + (n_off * stripe_width) + ( (i-1) * stripe_width * a_dim2 )) * 16_8
                istat =  cuda_memcpy( loc(bottom_border_send_buffer(1,1,i)), a_dev + dev_offset, stripe_width * bottom_msg_length * 16_8 , 2)
                t2_memcpy_time =MPI_Wtime()
                t0_memcpy_time = t0_memcpy_time + ( t2_memcpy_time -t1_memcpy_time)
 ...
 if(next_top_msg_length > 0) then
            ...
            if(my_prow > 0) then
               ...
               t0_mpi_wait_time = t0_mpi_wait_time+(t2_mpi_wait_time-t1_mpi_wait_time)
                 ...
                 t1_memcpy_time =MPI_Wtime()
                dev_offset = (0 + (a_off * stripe_width) + ( (i-1) * stripe_width *a_dim2 )) * 16_8
                istat =  cuda_memcpy( loc(top_border_send_buffer(1,1,i)), a_dev + dev_offset, stripe_width*nbw*16_8 ,2)
                   t2_memcpy_time =MPI_Wtime()
                t0_memcpy_time = t0_memcpy_time + (t2_memcpy_time-t1_memcpy_time)
     ...
     call MPI_Isend(top_border_send_buffer(1,1,i), nbw*stripe_width, MPI_COMPLEX16, my_prow-1, bottom_recv_tag, &
            ...
            ! Care that there are not too many outstanding
            ! top_recv_request's
                t1_mpi_wait_time =MPI_Wtime()
           ...
           ! receive and store final result
            ...
            do j = num_bufs_recvd, num_result_blocks-1
             nbuf = mod(j, num_result_buffers) + 1 ! buffer number to get this block
                ...
                ! If there is still work to do, just test for the next
                ! result request
                ! and leave the loop if it is not ready, otherwise wait
                ! for all
                ...
                num_blk = j*np_rows + my_prow ! global number of currentblock, 0 based
                do i = 1, min(na - num_blk*nblk, nblk)
                    q(j*nblk+i, 1:l_nev) = result_buffer(1:l_nev, i, nbuf)
                enddo

                ! Queue result buffer again if there are outstanding
                ! blocks left
    ...
    if(ANY(top_recv_request    /= MPI_REQUEST_NULL)) print *,'*** ERRORtop_recv_request ***',my_prow,my_pcol
...
!    if(my_prow==0 .and. my_pcol==0 .and. elpa_print_times) &
!    print *,'Time  for compute_kernel :', t0_outer_do_time,t0_inner_do_time,t0_cpu_code,t0_block_time, t0_compute_kernel *1000,time0, t0_mpi_outer_wait_time,t0_mpi_wait_time ,t0_memcpy_time
...
contains
    ...
    subroutine pack_row_group_complex(rows, n_offset, row_count)
        ...
        call launch_my_pack_c_kernel_complex(row_count, n_offset, max_idx, stripe_width,a_dim2, stripe_count, l_nev, a_dev, row_group_dev)
        istat =  cuda_memcpy( loc(rows(1, 1)), row_group_dev ,row_count * l_nev * 16_8 ,2)
    end subroutine
 ...
 subroutine unpack_row_group_complex(rows, n_offset, row_count)
        ...
        call launch_my_unpack_c_kernel_complex( row_count, n_offset,max_idx,stripe_width,a_dim2, stripe_count, l_nev, row_group_dev,a_dev)
    end subroutine
     subroutine unpack_and_prepare_row_group_complex(next_unpack_idx, force)
       ...
       subroutine compute_hh_trafo_complex_gpu(off, ncols, istripe)
      ...
      dev_offset = (0 + ( (  a_off + off-1 )* stripe_width) + ( (istripe - 1)*stripe_width*a_dim2 )) *16
       dev_offset_1 = (0 +  (  off-1 )* nbw) *16
        ...
        t1_compute_kernel =MPI_Wtime()
       call launch_compute_hh_trafo_c_kernel_complex(a_dev + dev_offset,bcast_buffer_dev + dev_offset_1, hh_tau_dev + dev_offset_2, nl, nbw,stripe_width, off,ncols)
