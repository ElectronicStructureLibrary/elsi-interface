...
main(int argc, char** argv) {
   ...
   set_up_blacsgrid_from_fortran(my_mpi_comm_world, &my_blacs_ctxt, &np_rows,
   ...
   /* These are NOT usable in C without calling the MPI_Comm_f2c function on
      them !! */
   my_mpi_comm_world = MPI_Comm_c2f(MPI_COMM_WORLD);
   mpierr = elpa_get_communicators(my_mpi_comm_world, my_prow, my_pcol,
   ...
   set_up_blacs_descriptor_from_fortran(na, nblk, my_prow, my_pcol, 
         np_rows, np_cols, &na_rows, &na_cols, sc_desc, my_blacs_ctxt, &info);
   ...
   a  = (double*) malloc(na_rows*na_cols*sizeof(double));
   z  = (double*) malloc(na_rows*na_cols*sizeof(double));
   as = (double*) malloc(na_rows*na_cols*sizeof(double));


   ev = (double*) malloc(na*sizeof(double));

   tmp1  = (double*) malloc(na_rows*na_cols*sizeof(double));
   tmp2  = (double*) malloc(na_rows*na_cols*sizeof(double));

   iseed = (int*) malloc(4096*sizeof(int));

   prepare_matrix_real_from_fortran(na, myid, na_rows, na_cols, sc_desc, 
         iseed, a, z, as);
   ...
   success = elpa_solve_evp_real_1stage(na, nev, na_cols, a, na_rows, ev, 
         z, na_rows, nblk, mpi_comm_rows, mpi_comm_cols);
   ...
   /* check the results */
   status = check_correctness_real_from_fortran(na, nev, na_rows, na_cols, 
         as, z, ev, sc_desc, myid, tmp1, tmp2);
