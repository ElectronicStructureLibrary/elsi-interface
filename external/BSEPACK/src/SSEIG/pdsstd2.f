      SUBROUTINE PDSSTD2( UPLO, N, A, IA, JA, DESCA, E, TAU, WORK,
     $                    LWORK, INFO )
*
      IMPLICIT NONE
*
...
*  =======
*
*  PDSSTD2 reduces a real skew-symmetric matrix sub( A ) to
...
*
*  PDSSTD2 is modified from PDSYTD2 in ScaLAPACK version 2.0.2.
*
...
*
      IF ( INFO .NE. 0 ) THEN
         CALL PXERBLA( ICTXT, 'PDSSTD2', -INFO )
...
*
                     CALL DSSMV( UPLO, J, TAUI, A( II+(JJ-1)*LDA ),
     $                    LDA, A( II+JK*LDA ), 1, ZERO, TAU( JJ ), 1 )
*
*                    Apply the transformation as a rank-2 update:
*                       A := A + v * x**T - x * v**T.
*
                     CALL DSSR2( UPLO, J, ONE, A( II+JK*LDA ), 1,
...
*
               ...
               WORK( N+1 ) = ZERO
*
               CALL DGEBS2D( ICTXT, 'C', ' ', 1, 2*N, WORK, 1 )
*
            ELSE
               CALL DGEBR2D( ICTXT, 'C', ' ', 1, 2*N, WORK, 1, IAROW,
     $              IACOL )
               DO J = 2, N
                  JN = JJ + J - 1
                  E( JN ) = WORK( J )
...
*
                     CALL DSSMV( UPLO, N-J, TAUI, A( IK+1+JK*LDA ),
...
*
               ...
               WORK( N ) = ZERO
*
               CALL DGEBS2D( ICTXT, 'C', ' ', 1, 2*N-1, WORK, 1 )
*
            ELSE
               CALL DGEBR2D( ICTXT, 'C', ' ', 1, 2*N-1, WORK, 1, IAROW,
     $              IACOL )
               DO J = 1, N - 1
                  JN = JJ + J - 1
                  E( JN ) = WORK( J )
