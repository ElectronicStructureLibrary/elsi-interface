      SUBROUTINE PSSSTRD( UPLO, N, A, IA, JA, DESCA, E, TAU, WORK,
     $                    LWORK, INFO )
*
      IMPLICIT NONE
*
...
*  =======
*
*  PSSSTRD reduces a real skew-symmetric matrix sub( A ) to
...
*          LWORK >= ( NP+1 ) * ( NP+1 ) + MAX( NB * ( NP+1 ), 3 * NB )
*
*          where NB = MB_A = NB_A,
...
*  =====================================================================
*
*  Level 3 ScaLAPACK-like routine.
*
*  PSSSTRD is modified from PSSYTRD in ScaLAPACK version 2.0.2.
*
...
*     .. External Subroutines ..
     ...
     $                   PSLASTD, PSSSR2K, PSSSTD2, PB_TOPGET,
     $                   PB_TOPSET, PXERBLA, PSELSET
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
...
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          REAL, ICHAR, MAX, MIN, MOD
*     ..
...
*
      ...
      ELSE
         ...
         IF ( INFO .EQ. 0 ) THEN
            ...
            NP = NUMROC( N, NB, MYROW, IAROW, NPROW )
            NC = NUMROC( N, NB, MYCOL, IACOL, NPCOL )
            NQ = MAX( 1, NUMROC( N+JA-1, NB, MYCOL, DESCA( CSRC_ ),
     $                NPCOL ) )
            LWMIN = (NP+1)*NC + MAX( (NP+1)*NB, 3*NB )
*
            WORK( 1 ) = REAL( LWMIN )
            LQUERY = ( LWORK .EQ. -1 )
...
*
      IF ( INFO .NE. 0 ) THEN
         CALL PXERBLA( ICTXT, 'PSSSTRD', -INFO )
         RETURN
      ELSE IF ( LQUERY ) THEN
         RETURN
      END IF
...
*
      IF ( N .LE. 1 ) RETURN
*
      CALL PB_TOPGET( ICTXT, 'C', 'C', COLCTOP )
      CALL PB_TOPGET( ICTXT, 'C', 'R', ROWCTOP )
      CALL PB_TOPSET( ICTXT, 'C', 'C', '1-tree' )
      CALL PB_TOPSET( ICTXT, 'C', 'R', '1-tree' )
*
      IPW = NP * NB + 1
*
      IF ( UPPER ) THEN
...
*
     ...
     $        NB, MYCOL, DESCA( CSRC_ ), NPCOL ), ICTXT, MAX( 1, NP ) )
*
         DO K = N-KK+1, NB+1, -NB
            JB = MIN( N-K+1, NB )
            I = IA + K - 1
...
*           the matrix.
*
            CALL PSLASTD( UPLO, K+JB-1, JB, A, IA, JA, DESCA, E, TAU,
     $           WORK, 1, 1, DESCW, WORK( IPW ) )
*
*           Update the unreduced submatrix A(IA:I-1,JA:J-1), using an
*           update of the form:
*           A(IA:I-1,JA:J-1) := A(IA:I-1,JA:J-1) + V*W**T - W*V**T.
*
            CALL PSSSR2K( UPLO, 'N', K-1, JB, ONE, A, IA, J, DESCA,
     $           WORK, 1, 1, DESCW, ONE, A, IA, JA, DESCA, WORK( IPW ) )
*
...
*
         CALL PSSSTD2( UPLO, MIN( N, NB ), A, IA, JA, DESCA, E, TAU,
     $        WORK, LWORK, IINFO )
*
      ELSE
*
...
*           of the matrix.
*
            CALL PSLASTD( UPLO, N-K+1, NB, A, I, J, DESCA, E, TAU,
...
*           A(I+NB:IA+N-1,I+NB:IA+N-1) + V*W**T - W*V**T.
*
            CALL PSSSR2K( UPLO, 'N', N-K-NB+1, NB, ONE, A, I+NB, J,
     $           DESCA, WORK, K+NB, 1, DESCW, ONE, A, I+NB, J+NB,
     $           DESCA, WORK( IPW ) )
*
...
*        Use unblocked code to reduce the last or only block.
*
         CALL PSSSTD2( UPLO, KK, A, IA+K-1, JA+K-1, DESCA, E, TAU,
     $        WORK, LWORK, IINFO )
      END IF
*
      CALL PB_TOPSET( ICTXT, 'C', 'C', COLCTOP )
      CALL PB_TOPSET( ICTXT, 'C', 'R', ROWCTOP )
*
      WORK( 1 ) = REAL( LWMIN )
*
      RETURN
