      SUBROUTINE PDLASTD( UPLO, N, NB, A, IA, JA, DESCA, E, TAU, W, IW,
     $                    JW, DESCW, WORK )
*
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      INTEGER            DESCA( * ), DESCW( * )
      DOUBLE PRECISION   A( * ), E( * ), TAU( * ), W( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PDLASTD reduces NB rows and columns of a real skew-symmetric
...
*  sub( A ).
*
*  If UPLO = 'U', PDLASTD reduces the last NB rows and columns of a
*  matrix, of which the upper triangle is supplied;
*  if UPLO = 'L', PDLASTD reduces the first NB rows and columns of a
*  matrix, of which the lower triangle is supplied.
*
*  This is an auxiliary routine called by PDSSTRD.
*
*  Notes
*  =====
*
...
*
*  PDLASTD is modified from PDLATRD in ScaLAPACK version 2.0.2.
*
...
*           Update A(IA:I-1,I).
*
            CALL PDGEMV( 'N', K-1, N-K, ONE, A, IA, J+1, DESCA, W,
     $           IW+K-1, JW+KW, DESCW, DESCW( M_ ), ONE, A, IA, J,
     $           DESCA, 1 )
            CALL PDGEMV( 'N', K-1, N-K, -ONE, W, IW, JW+KW, DESCW, A, I,
     $           J+1, DESCA, DESCA( M_ ), ONE, A, IA, J, DESCA, 1 )
            IF ( N-K .GT. 0 ) CALL PDELSET( A, I, J+1, DESCA, E( JP ) )
*
...
*
            CALL PDSSMV( 'U', K-1, ONE, A, IA, JA, DESCA, A, IA, J,
     $           DESCA, 1, ZERO, W, IW, JW+KW-1, DESCW, 1, WORK )
*
            JWK = MOD( K-1, DESCWK( NB_ ) ) + 2
            CALL PDGEMV( 'T', K-1, N-K, ONE, W, IW, JW+KW, DESCW, A, IA,
     $           J, DESCA, 1, ZERO, WORK, 1, JWK, DESCWK, DESCWK( M_ ) )
            CALL PDGEMV( 'N', K-1, N-K, ONE, A, IA, J+1, DESCA, WORK, 1,
     $           JWK, DESCWK, DESCWK( M_ ), ONE, W, IW, JW+KW-1, DESCW,
     $           1 )
            CALL PDGEMV( 'T', K-1, N-K, ONE, A, IA, J+1, DESCA, A, IA,
     $           J, DESCA, 1, ZERO, WORK, 1, JWK, DESCWK, DESCWK( M_ ) )
            CALL PDGEMV( 'N', K-1, N-K, -ONE, W, IW, JW+KW, DESCW, WORK,
...
*
            CALL PDGEMV( 'N', N-K, K-1, ONE, A, I+1, JA, DESCA, W,
     $           IW+K-1, JW, DESCW, DESCW( M_ ), ONE, A, I+1, J, DESCA,
     $           1 )
            CALL PDGEMV( 'N', N-K, K-1, -ONE, W, IW+K, JW, DESCW,
     $           A, I, JA, DESCA, DESCA( M_ ), ONE, A, I+1, J, DESCA,
     $           1 )
            IF ( K .GT. 1 ) CALL PDELSET( A, I, J-1, DESCA, -E( JP ) )
*
*           Generate elementary reflector H(j) to annihilate
*           A(I+2:IA+N-1,J).
*
...
*
            CALL PDSSMV( 'L', N-K, ONE, A, I+1, J+1, DESCA, A, I+1, J,
     $           DESCA, 1, ZERO, W, IW+K, JW+K-1, DESCW, 1, WORK )
*
            CALL PDGEMV( 'T', N-K, K-1, ONE, W, IW+K, JW, DESCW, A, I+1,
     $           J, DESCA, 1, ZERO, WORK, 1, 1, DESCWK, DESCWK( M_ ) )
            CALL PDGEMV( 'N', N-K, K-1, ONE, A, I+1, JA, DESCA, WORK, 1,
     $           1, DESCWK, DESCWK( M_ ), ONE, W, IW+K, JW+K-1, DESCW,
     $           1 )
            CALL PDGEMV( 'T', N-K, K-1, ONE, A, I+1, JA, DESCA, A, I+1,
     $           J, DESCA, 1, ZERO, WORK, 1, 1, DESCWK, DESCWK( M_ ) )
            CALL PDGEMV( 'N', N-K, K-1, -ONE, W, IW+K, JW, DESCW, WORK,
