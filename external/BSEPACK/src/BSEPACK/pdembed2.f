      SUBROUTINE PDEMBED2( N, A, IA, JA, DESCA, B, IB, JB, DESCB, H, IH,
     $                     JH, DESCH, WORK )
*
      IMPLICIT NONE
*
...
*     .. Array Arguments ..
      ...
      DOUBLE PRECISION   A( * ), B( * ), H( * ), WORK( * )
*     ..
*
*  Purpose
*  =======
*
*  PDEMBED2() is an auxiliary routine called by PDBSEIG().
...
*
*  H       (local output) DOUBLE PRECISION pointer into the local memory
...
*     .. External Subroutines ..
      EXTERNAL           PDGEADD, PDTRADD, PDLACPY, PDLASET,
     $                   PDELGET, PDELSET
*     ..
...
*
         CALL PDLASET( 'L', N, N, ZERO, ZERO, WORK, IA, JA, DESCA )
         CALL PDLACPY( 'L', N-1, N-1, A, IA + 1, JA, DESCA, WORK,
     $        IA + 1, JA, DESCA )
         CALL PDTRADD( 'U', 'T', N, N, ONE, WORK, IA, JA, DESCA, ZERO,
     $        H, IH, JH, DESCH )
         CALL PDTRADD( 'L', 'N', N, N, ONE, A, IA, JA, DESCA, ZERO,
     $        H, IH, JH, DESCH )
         CALL PDGEADD( 'T', N, N, -ONE, H, IH, JH, DESCH, ZERO,
     $        H, IH + N, JH + N, DESCH )
*
         CALL PDLASET( 'L', N, N, ZERO, ZERO, WORK, IB, JB, DESCB )
         CALL PDLACPY( 'L', N-1, N-1, B, IB + 1, JB, DESCA, WORK,
     $        IB + 1, JB, DESCB )
         CALL PDTRADD( 'U', 'T', N, N, ONE, WORK, IB, JB, DESCB, ZERO,
     $        H, IH, JH + N, DESCH )
         CALL PDTRADD( 'L', 'N', N, N, ONE, B, IB, JB, DESCB, ZERO,
     $        H, IH, JH + N, DESCH )
         CALL PDGEADD( 'T', N, N, -ONE, H, IH, JH + N, DESCH, ZERO,
     $        H, IH + N, JH, DESCH )
*
      ELSE
*
...
*
         DO J = 0, N-1
            DO I = J, N-1
               CALL PDELGET( 'A', ' ', DTMP, A, IA + I, JA + J, DESCA )
               CALL PDELSET( H, IH + I, JH + J, DESCH, DTMP )
               CALL PDELSET( H, IH + J, JH + I, DESCH, DTMP )
               CALL PDELSET( H, IH + N+I, JH + N+J, DESCH, -DTMP )
               CALL PDELSET( H, IH + N+J, JH + N+I, DESCH, -DTMP )
               CALL PDELGET( 'A', ' ', DTMP, B, IB + I, JB + J, DESCB )
               CALL PDELSET( H, IH + I, JH + N+J, DESCH, DTMP )
               CALL PDELSET( H, IH + J, JH + N+I, DESCH, DTMP )
               CALL PDELSET( H, IH + N+I, JH + J, DESCH, -DTMP )
               CALL PDELSET( H, IH + N+J, JH + I, DESCH, -DTMP )
            END DO
         END DO
      END IF
*
