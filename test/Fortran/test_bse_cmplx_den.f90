...
subroutine test_bse_cmplx_den(comm,solver,a_file,b_file)
   ...
   complex(kind=r8), allocatable :: mat_a(:,:)
   complex(kind=r8), allocatable :: mat_b(:,:)
   ...
   if(myid == 0) then
      ...
      write(*,*)
      if(solver == 8) then
         write(*,"(2X,A)") "Now start testing  elsi_bse_complex + BSEPACK"
      end if
      write(*,*)
   end if
   ...
   call elsi_read_mat_complex(rwh,a_file,mat_a)
   call elsi_read_mat_complex(rwh,b_file,mat_b)
   ...
   ! Solve
   call elsi_bse_complex(eh,mat_a,mat_b,eval,evec)
