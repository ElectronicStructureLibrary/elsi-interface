...
subroutine qr_pdlarfb_kernel_local_double(m,n,k,a,lda,v,ldv,t,ldt,z,ldz)
...
! input variables (local)
    ...
    real(kind=rk8)    :: a(lda,*),v(ldv,*),t(ldt,*),z(ldz,*)
...
! local variables
    real(kind=rk8)    :: t11
    real(kind=rk8)    :: t12,t22,sum1,sum2
    real(kind=rk8)    :: t13,t23,t33,sum3
    real(kind=rk8)    :: sum4,t44
    real(kind=rk8)    :: y1,y2,y3,y4
    real(kind=rk8)    :: a1
...
subroutine qr_pdlarft_merge_kernel_local_double(oldk,k,t,ldt,yty,ldy)
...
! local scalars for optimized versions
    ...
    real(kind=rk8)    :: t11
    real(kind=rk8)    :: yty1,yty2,yty3,yty4,yty5,yty6,yty7,yty8
    real(kind=rk8)    :: reg01,reg02,reg03,reg04,reg05,reg06,reg07,reg08
    real(kind=rk8)    :: final01,final02,final03,final04,final05,final06,final07,final08
...
subroutine qr_tmerge_set_kernel_double(k,blocksize,t,ldt,yty,ldy)
...
! start with latest T matrix part and add older ones
        ...
        if (remainder .gt. 0) then
            ...
            call qr_pdlarft_merge_kernel_local_double(blocksize,remainder,t(toffset,toffset),ldt,yty(1,yty_column),ldy)
            ...
            call qr_pdlarft_merge_kernel_local_double(blocksize,blocksize,t(toffset,toffset),ldt,yty(1,yty_column),ldy)
        ...
        do while (current_block .lt. nr_blocks)
            ...
            call qr_pdlarft_merge_kernel_local_double(blocksize,oldk,t(toffset,toffset),ldt,yty(toffset,yty_column),ldy)
...
subroutine qr_tmerge_tree_kernel_double(k,blocksize,treeorder,t,ldt,yty,ldy)
...
! local scalars
    ...
    if (treeorder .eq. 1) then
        ...
        call qr_tmerge_set_kernel_double(k,blocksize,t,ldt,yty,ldy)
...
!print *,'final merging with remainder',temp_blocksize,k,remaining_size,yty_column
                    ...
                    call qr_tmerge_set_kernel_double(k,temp_blocksize,t,ldt,yty(1,yty_column),ldy)
...
!print *,'set merging', toffset, yty_column,remainder
                    ...
                    call qr_tmerge_set_kernel_double(remainder,temp_blocksize,t(toffset,toffset),ldt,yty(toffset,yty_column),ldy)
...
!print *,'single+set merging',yty_remainder,total_remainder,remainder
                        ...
                        call qr_pdlarft_merge_kernel_local_double(remainder,total_remainder,t(1,1),ldt,yty(1,yty_remainder),ldy)
...
!print *,'recursive merging', toffset, yty_column,setsize
                ...
                call qr_tmerge_set_kernel_double(setsize,temp_blocksize,t(toffset,toffset),ldt,yty(toffset,yty_column),ldy)
...
subroutine qr_dlarft_kernel_double(n,tau,yty,ldy,t,ldt)
