To check for possible ill-conditioning of the basis set, ELSI requires the usage 
of a check_singularity subroutine.  As a short-term solution, this is done by a
redistributed ELPA 2016.11.001.pre package modified to include an additional 
check_singularity module including two subroutines,  
elpa_check_singularity_{real,complex}_double.  Module permissions require that 
we rebuild ELPA from source to use this module.

We note that this modified form of ELPA is *only* called by the 
elpa_check_singularity_{real,complex}_double subroutines.  All other calls to ELPA
within ELSI (e.g. the solution of the Kohn-Sham eigenvalue equations) use the "main" 
ELPA solver, either built from the main source code redistributed in 
${ELSI_HOME}/src/ELPA or provided as an externally linked library by the user.  We 
anticipate that this short-term solution will be removed for the July 2017 release 
of ELSI.

Below is the original ELPA README file.
