...
subroutine qr_pdlarfb_1dcomm_double(m,mb,n,k,a,lda,v,ldv,tau,t,ldt,baseidx,idx,rev,mpicomm,work,lwork)
    use precision_cs
    use qr_utils_mod_cs
    ...
    if (k .eq. 1) then
        call qr_pdlarfl_1dcomm_double(v,1,baseidx,a,lda,tau(1), &
                                work,lwork,m,n,idx,mb,rev,mpicomm)
        return
    else if (k .eq. 2) then
        call qr_pdlarfl2_tmatrix_1dcomm_double(v,ldv,baseidx,a,lda,t,ldt, &
                                 work,lwork,m,n,idx,mb,rev,mpicomm)
        return
    end if
...
#endif
    call qr_pdlarfb_kernel_local_double(localsize,n,k,a(offset,1),lda,v(baseoffset,1),ldv,t,ldt,work(1,n+1),k)
end subroutine qr_pdlarfb_1dcomm_double

! generalized pdlarfl2 version
! TODO: include T merge here (seperate by "old" and "new" index)
subroutine qr_pdlarft_pdlarfb_1dcomm_double(m,mb,n,oldk,k,v,ldv,tau,t,ldt,a,lda,baseidx,rev,mpicomm,work,lwork)
    use precision_cs
    use qr_utils_mod_cs
...
#endif
end subroutine qr_pdlarft_pdlarfb_1dcomm_double

subroutine qr_pdlarft_set_merge_1dcomm_double(m,mb,n,blocksize,v,ldv,t,ldt,baseidx,rev,mpicomm,work,lwork)
    use precision_cs
    use qr_utils_mod_cs
    ...
    work(1:n,n+1:n+1+n-1) = work(1:n,1:n)
        ...
        call qr_tmerge_set_kernel_double(n,blocksize,t,ldt,work(1,n+1+offset),n)

end subroutine qr_pdlarft_set_merge_1dcomm_double

subroutine qr_pdlarft_tree_merge_1dcomm_double(m,mb,n,blocksize,treeorder,v,ldv,t,ldt,baseidx,rev,mpicomm,work,lwork)
    use precision_cs
    use qr_utils_mod_cs
    ...
    work(1:n,n+1:n+1+n-1) = work(1:n,1:n)
        ...
        offset = mod(n,blocksize)
        if (offset .eq. 0) offset=blocksize
        call qr_tmerge_tree_kernel_double(n,blocksize,treeorder,t,ldt,work(1,n+1+offset),n)
...
end subroutine qr_pdlarfl_1dcomm_double
...
subroutine qr_pdlarfl2_tmatrix_1dcomm_double(v,ldv,baseidx,a,lda,t,ldt,work,lwork,m,n,idx,mb,rev,mpicomm)
    ...
    real(kind=rk8)    :: v(ldv,*),a(lda,*),work(*),t(ldt,*)
...
end subroutine qr_pdlarfl2_tmatrix_1dcomm_double

! generalized pdlarfl2 version
! TODO: include T merge here (seperate by "old" and "new" index)
subroutine qr_tmerge_pdlarfb_1dcomm_double(m,mb,n,oldk,k,v,ldv,t,ldt,a,lda,baseidx,rev,updatemode,mpicomm,work,lwork)
    use precision_cs
    use qr_utils_mod_cs
    ...
    ! input variables (local)
    integer(kind=ik) :: ldv,ldt,lda,lwork
    real(kind=rk8)    :: v(ldv,*),t(ldt,*),work(*),a(lda,*)
    ...
    tgenoffset = recvoffset+tgenoffset
        ...
        if (oldk .gt. 0) then
            call qr_pdlarft_merge_kernel_local_double(oldk,k,t,ldt,work(mergeoffset),mergelda)

            if (localsize .gt. 0) then
                if (updatemode .eq. ichar('I')) then

                    ! update matrix (pdlarfb) with complete T
                    call qr_pdlarfb_kernel_local_double(localsize,n,k+oldk,a(offset,1),lda,v(baseoffset,1),ldv,t(1,1),ldt, &
                                                 work(updateoffset),updatelda)
                else
                    ! update matrix (pdlarfb) with small T (same as update with no old T TODO)
                    call qr_pdlarfb_kernel_local_double(localsize,n,k,a(offset,1),lda,v(baseoffset,1),ldv,t(1,1),ldt, &
                                                 work(updateoffset),updatelda)
                end if
            end if
        else
...
end subroutine qr_tmerge_pdlarfb_1dcomm_double

#ifdef WANT_SINGLE_PRECISION_REAL

#undef DOUBLE_PRECISION_REAL

subroutine qr_pdlarfb_1dcomm_single(m,mb,n,k,a,lda,v,ldv,tau,t,ldt,baseidx,idx,rev,mpicomm,work,lwork)
    use precision_cs
    use qr_utils_mod_cs
    ...
    ! input variables (local)
    integer(kind=ik)  :: lda,ldv,ldt,lwork
    real(kind=rk4)     :: a(lda,*),v(ldv,*),tau(*),t(ldt,*),work(k,*)
    ...
    if (k .eq. 1) then
        call qr_pdlarfl_1dcomm_single(v,1,baseidx,a,lda,tau(1), &
                                work,lwork,m,n,idx,mb,rev,mpicomm)
        return
    else if (k .eq. 2) then
        call qr_pdlarfl2_tmatrix_1dcomm_single(v,ldv,baseidx,a,lda,t,ldt, &
                                 work,lwork,m,n,idx,mb,rev,mpicomm)
        return
    end if
    ...
    work(1:k*n,n+1) = work(1:k*n,1)
#endif
    call qr_pdlarfb_kernel_local_single(localsize,n,k,a(offset,1),lda,v(baseoffset,1),ldv,t,ldt,work(1,n+1),k)
end subroutine qr_pdlarfb_1dcomm_single

! generalized pdlarfl2 version
! TODO: include T merge here (seperate by "old" and "new" index)
subroutine qr_pdlarft_pdlarfb_1dcomm_single(m,mb,n,oldk,k,v,ldv,tau,t,ldt,a,lda,baseidx,rev,mpicomm,work,lwork)
    use precision_cs
    use qr_utils_mod_cs
    ...
    ! input variables (local)
    integer(kind=ik)  :: ldv,ldt,lda,lwork
    real(kind=rk4)     :: v(ldv,*),tau(*),t(ldt,*),work(k,*),a(lda,*)
    ...
    work(1:sendsize,recvoffset) = work(1:sendsize,sendoffset)
        ...
        ! Z' = T * Z'
#ifdef DOUBLE_PRECISION_REAL
        call dtrmm("Left","Upper","Notrans","Nonunit",k,n,1.0_rk8,t,ldt,work(1,recvoffset+k),k)

        ! A = A - Y * V'
        call dgemm("Notrans","Notrans",localsize,n,k,-1.0_rk8,v(baseoffset,1),ldv,work(1,recvoffset+k),k,1.0_rk8,a(offset,1),lda)
#else
        call strmm("Left","Upper","Notrans","Nonunit",k,n,1.0_rk4,t,ldt,work(1,recvoffset+k),k)
...
#endif
end subroutine qr_pdlarft_pdlarfb_1dcomm_single

subroutine qr_pdlarft_set_merge_1dcomm_single(m,mb,n,blocksize,v,ldv,t,ldt,baseidx,rev,mpicomm,work,lwork)
    use precision_cs
    use qr_utils_mod_cs
    ...
    ! input variables (local)
    integer(kind=ik)  :: ldv,ldt,lwork
    real(kind=rk4)     :: v(ldv,*),t(ldt,*),work(n,*)
    ...
    work(1:n,n+1:n+1+n-1) = work(1:n,1:n)
        ...
        offset = mod(n,blocksize)
        if (offset .eq. 0) offset=blocksize
        call qr_tmerge_set_kernel_single(n,blocksize,t,ldt,work(1,n+1+offset),n)

end subroutine qr_pdlarft_set_merge_1dcomm_single

subroutine qr_pdlarft_tree_merge_1dcomm_single(m,mb,n,blocksize,treeorder,v,ldv,t,ldt,baseidx,rev,mpicomm,work,lwork)
    use precision_cs
    use qr_utils_mod_cs
    ...
    work(1:n,n+1:n+1+n-1) = work(1:n,1:n)
        ...
        offset = mod(n,blocksize)
        if (offset .eq. 0) offset=blocksize
        call qr_tmerge_tree_kernel_single(n,blocksize,treeorder,t,ldt,work(1,n+1+offset),n)
...
end subroutine qr_pdlarfl_1dcomm_single
...
subroutine qr_pdlarfl2_tmatrix_1dcomm_single(v,ldv,baseidx,a,lda,t,ldt,work,lwork,m,n,idx,mb,rev,mpicomm)
    ...
    ! input variables (local)
    integer(kind=ik) :: ldv,lda,lwork,baseidx,ldt
    real(kind=rk4)    :: v(ldv,*),a(lda,*),work(*),t(ldt,*)
...
end subroutine qr_pdlarfl2_tmatrix_1dcomm_single

! generalized pdlarfl2 version
! TODO: include T merge here (seperate by "old" and "new" index)
subroutine qr_tmerge_pdlarfb_1dcomm_single(m,mb,n,oldk,k,v,ldv,t,ldt,a,lda,baseidx,rev,updatemode,mpicomm,work,lwork)
    use precision_cs
    use qr_utils_mod_cs
    ...
    ! input variables (local)
    integer(kind=ik) :: ldv,ldt,lda,lwork
    real(kind=rk4)    :: v(ldv,*),t(ldt,*),work(*),a(lda,*)
    ...
    tgenoffset = recvoffset+tgenoffset
        ...
        if (oldk .gt. 0) then
            call qr_pdlarft_merge_kernel_local_single(oldk,k,t,ldt,work(mergeoffset),mergelda)

            if (localsize .gt. 0) then
                if (updatemode .eq. ichar('I')) then

                    ! update matrix (pdlarfb) with complete T
                    call qr_pdlarfb_kernel_local_single(localsize,n,k+oldk,a(offset,1),lda,v(baseoffset,1),ldv,t(1,1),ldt, &
                                                 work(updateoffset),updatelda)
                else
                    ! update matrix (pdlarfb) with small T (same as update with no old T TODO)
                    call qr_pdlarfb_kernel_local_single(localsize,n,k,a(offset,1),lda,v(baseoffset,1),ldv,t(1,1),ldt, &
                                                 work(updateoffset),updatelda)
                end if
            end if
        else
...
end subroutine qr_tmerge_pdlarfb_1dcomm_single

#endif /* WANT_SINGLE_PRECISION_REAL */
end module elpa_pdlarfb_cs
